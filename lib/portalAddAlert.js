...
async function addAlert(req, res) {
    ...
    try {
        ...
        // WhatsApp "alerta ativado" (só se o cliente tem telefone = contratou WhatsApp).
        await notifyAlertsActivated({
            alerts: [{ restaurant, date, partySize }],
            phones,
            lang: pp.lang === 'en' ? 'en' : 'pt',
        }).catch((e) => console.error('add-alert: falha WhatsApp ativado:', e.message));
