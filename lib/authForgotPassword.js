...
async function authForgotPassword(req, res) {
    ...
    try {
        ...
        if (user) {
            ...
            if (resetUrl) {
                const logoFromEnv = process.env.EMAIL_LOGO_URL && String(process.env.EMAIL_LOGO_URL).trim();
                const logoUrl = logoFromEnv || `${base}/favicon.png`;
                try {
                    await sendPasswordResetEmail({ to: email, resetUrl, logoUrl });
                } catch (mailErr) {
                    console.error('[auth] Falha ao enviar e-mail (token válido no servidor):', mailErr);
                }
            } else {
                console.warn(
