...
const ITEM = {
    ...
    alertUrgent: { pt: (r, d) => `Alerta URGENTE (mesmo dia): ${r} — ${d}`, en: (r, d) => `URGENT alert (same day): ${r} — ${d}` },
...
function buildAlerts(cart, phones, currency = 'brl', lang = 'pt') {
    ...
    dates.forEach((d, i) => {
        const urgent = daysAhead(d.dateString) === 0; // mesmo dia = alerta urgente
        const price = priceByDaysAhead(daysAhead(d.dateString));
        total += price;
        lineItems.push(line(itemName(urgent ? 'alertUrgent' : 'alert', lang, restaurant, d.dateString), price, 1, currency));
        alerts.push({
            id: i + 1,
            restaurant,
            park,
            ...
            status: 'active',
            urgent, // alerta de mesmo dia: worker monitora com frequência maior
