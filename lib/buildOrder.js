...
function buildOrder(cart) {
    ...
    let order;
    if (planType === 'alerts') order = buildAlerts(cart, phones, currency);
    else if (planType === 'subscription') order = buildSubscription(cart, phones, currency);
    else if (planType === 'concierge') order = buildConcierge(cart, phones, currency);
