...
async function authPortalRegister(req, res) {
    ...
    try {
        ...
        // Materializa os alertas comprados em documentos Alert (fonte da verdade
        // do worker de monitoramento). Falha aqui não deve derrubar o registro.
        try {
            await syncAlertsFromUser({ email, portalPayload, userId: user && user._id });
        } catch (syncErr) {
            console.error('portal-register: falha ao sincronizar alertas:', syncErr);
        }
