...
const { sendPurchaseEmail, sendConciergeTeamEmail } = require('./resendMail');
const { notifyAlertsActivated, notifyConciergeTeamWhatsApp } = require('./whatsappActivation');

/** Telefones podem vir como strings ou objetos {full}. Normaliza p/ lista de strings. */
...
async function fulfill(session) {
    ...
    // WhatsApp "alerta ativado": só quem informou telefone (= contratou WhatsApp).
    // Envia por alerta RECÉM-comprado (orderData.alerts); assinatura tem alerts:[]
    // aqui e ativa no portal (portalAddAlert cuida daquele caso). Best-effort.
    await notifyAlertsActivated({
        alerts: orderData.alerts || [],
        phones: extractPhones(orderData.phones),
        lang: orderData.lang,
    }).catch((e) => console.error('[webhook] falha WhatsApp ativado:', e.message));

    // Concierge: avisa a EQUIPE (execução manual) com os detalhes da viagem,
    // por e-mail e também por WhatsApp (se configurado). Best-effort.
    if (orderData.plan && orderData.plan.type === 'concierge') {
        const ci = orderData.conciergeInfo || {};
        const phone = ci.phone || (Array.isArray(orderData.phones) && orderData.phones[0]) || '';
        const teamInfo = {
            planName: orderData.plan.name,
            customerName: orderData.name || ci.contactName,
            phone,
            tripDateStart: ci.tripDateStart,
            tripDateEnd: ci.tripDateEnd,
            partySize: ci.partySize,
        };
        await sendConciergeTeamEmail({
            ...teamInfo,
            customerEmail: email,
            price: orderData.plan.price,
            currency: pending.currency,
            lang: orderData.lang,
        }).catch((e) => console.error('[webhook] falha aviso concierge à equipe (e-mail):', e.message));
        await notifyConciergeTeamWhatsApp(teamInfo)
            .catch((e) => console.error('[webhook] falha aviso concierge à equipe (whatsapp):', e.message));
    }
    ...
    // Guarda a senha temporária p/ revelação ÚNICA na tela de sucesso (cliente novo).
    if (tempPassword) pending.firstAccessPassword = tempPassword;
    await pending.save();
